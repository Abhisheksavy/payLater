...
const Dashboard = () => {
  ...
  const getTierInfo = (points: number) => {
    ...
    if (points >= 5000) return { tier: "Gold", progress: ((points - 5000) / 5000) * 100, nextTier: "Platinum", pointsNeeded: 10000 - points };
    if (points >= 2000) return { tier: "Silver", progress: ((points - 2000) / 3000) * 100, nextTier: "Gold", pointsNeeded: 5000 - points };
    return { tier: "Bronze", progress: (points / 2000) * 100, nextTier: "Silver", pointsNeeded: 2000 - points };
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">+{monthlyPoints} this month</p>
              </CardContent>
            </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium text-primary">{monthlyPoints}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Bills Paid</span>
                    <span className="font-medium">{bills.filter(b => b.status === 'paid').length}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Categories</span>
                    <span className="font-medium">{Object.keys(categorySpending).length}</span>
                  </div>
                </CardContent>
              </Card>
                    ...
                    <div className="space-y-3">
                      {recentActivity.map((bill) => (
                        <div key={bill.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                          <div>
                            <p className="font-medium text-sm">{bill.name}</p>
                            <p className="text-xs text-muted-foreground">{bill.company}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-sm font-medium">+{bill.rewards} pts</p>
